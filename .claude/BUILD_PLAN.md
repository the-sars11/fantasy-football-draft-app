<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    "UX-S1 + UX-S2 + UX-S2.5 + UX-S3 done. UX-S3 rebuilt the whole Research tab to GRIDIRON per Section 9 blueprints 9.1-9.4: Research landing (Run Research hero + AI-cost tap guard + jump rows), Player Browser (search/filter hero, count header, target/avoid, deep-link empty), Draft Board (single-Nasties board, meta strip, graceful empty/error), Strategies (active-strategy hero + budget-by-position, demoted Dry Run). Double-header bug fixed in players/page.tsx + strategies/page.tsx. Verified typecheck 0 + lint 0 on all changed files, 8 Playwright screenshots (each screen x mobile 390 + desktop 1440) with real connected data. NEXT: UX-S4 Draft tab = live auction room ONLY + FF-314 auto-connect / 4-state connection chip / interruption+retry / manual fallback. See .claude/UX_OVERHAUL_2026-08.md.",
