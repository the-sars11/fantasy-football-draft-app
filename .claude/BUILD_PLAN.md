<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    "UX-S1 + UX-S2 + UX-S2.5 + UX-S3 + UX-S4 done. UX-S4 rebuilt the Draft tab as the live auction room ONLY and shipped FF-314 cross-device auto-connect. /draft is now the pre-Go-Live screen (blueprint 9.5): ONE hero = large 4-state ConnectionStatusPill + plain-words status, primary Go Live (enabled only when the auctioneer feed is detected), secondary Start in Manual mode, pre-flight card + Edit-in-Setup, loading/no-league/error states. Live room (9.6) is full-screen — app-shell.tsx hides sidebar + mobile top header + bottom tab bar + swipe-carousel on /draft/live, with a single Leave affordance. FF-314: new server-side proxy route /api/auctioneer-feed (CORS dodge, Upstash draft-current) + use-remote-auctioneer-feed.ts (3s poll, exp backoff, dedup, retry) folded into use-draft-feed.ts as a new SOURCE; Tyler's snake/Sheets/manual path untouched. Verified typecheck 0 + lint 0 on all changed files, dev server clean, render proof (real Nasties data) for pre-Go-Live + live room on mobile 390 + desktop 1440; only free Upstash reads fired (no paid /api/research or /api/strategies/propose). NEXT: UX-S5 Setup tab + data correctness — league config with LOCKED Nasties defaults auto-seeded + editable, draft setup, demo, run-history log, Account from real signed-in user (remove hardcoded propermuse.co email), remove Draft sounds + snake + keeper, fix 'failed to fetch leagues'. See .claude/UX_OVERHAUL_2026-08.md.",
