...
> - **F3 [P3, observed]** `/prep/players` still renders a "K" (kicker) filter tab even though the Nasties is a no-kicker league (`roster_slots.k: 0`). Cosmetic; logged for a filter-list pass gated on the active league's roster slots. Not a crash.
