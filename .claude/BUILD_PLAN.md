...
- **[ ] OPEN DECISION — sim opponents anchor on national ceiling, not room price (awaiting Joe).** The Monte-Carlo field bids off `nominated.ceiling` (national upside), so contested studs clear ABOVE the ledger in-sim (Studs & Duds lands Gibbs at sim-avg $117 vs $76 room). `league-opponents.ts` header says opponents *should* bid off room price (`expectedCost x vsRoom`); `sim-engine.ts` uses `ceiling x tilt`. Recommendation: align sim opponents to room price so the simulated market matches the ledger + the new target-price advice. Changes every win-rate -> needs Joe's go before touching the sim.
