<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    "W-track [Joe-approved 2026-08-21]: wire the headless research engine + a draft-day decide->lock->phone workflow INTO the app UI (see 'W-track' section). W0 [Opus] DONE 2026-08-21 = dataset contract (dataset-types.ts) + storage seam (research:publish -> research_runs kind:'dataset' + GET /api/research-dataset + use-research-dataset hook); proven end-to-end, McCaffrey $60 (room $67, 0.894x) round-trips through the DB. NEXT = W1 [Sonnet]: Strategy Leaderboard screen (SHIELD mockup + Joe sign-off before build).",
...
- [ ] **W1 [Sonnet] · output (Design+QA) — Strategy Leaderboard screen.** New `prep/leaderboard/{page,client}.tsx` + a `/prep` hub destination row (D2 pattern). Ranks `dataset.strategies` by `sim.grade.meanWins`: record hero, why-it-wins, `key_targets` w/ durability prices, expandable modal rosters, a `dataset.studCombos` section. Reuse `sim-results-cards.tsx`. SHIELD mockup + Joe sign-off first. Done-when: mobile screenshot with real 400-run records + McCaffrey discount + a stud combo.
- [ ] **W2 [Sonnet] · output+shared — League Intel + player-screen upgrades.** `league-intel-panel.tsx` (HOT/COOL inflation + owner leans) on leaderboard + `/prep/board`; merge dataset per-player enrichment (`valueBand`/`tags`/`read`/`landProbability`/`ceilingValue`/`expectedRoomPrice`/`valueGap`) over `/prep/players` + `/prep/board` rows when a dataset exists (additive fallback to client compute); attach `durabilityPriceFactor` per `EnrichedPlayer` in `research-run.ts`. Done-when: mobile screenshots showing land odds, durability price, HOT/COOL bands.
