<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    ...
    "D6b [Sonnet+Opus, L]: NOT STARTED. Align shipped D6 to locked spec v5 (DEC-2a) + wire the deterministic Read signals (DEC-2b). UI GAPS vs v5 (Sonnet, pure UI, ref d6_cockpit_mockup_v5.html): (1) on-block market BAND = steel-blue MARKET range ($low-$high) as a positioned track with a SEPARATE red TARGET marker at your price (may sit inside/at-low/below band by strategy) - replaces today's plain 'Your range' text with inverted blue/red meaning (on-the-block-card.tsx:191-210); (2) pin a labeled target price ('$34 tgt') in the on-block COLLAPSED summary next to name+chip - today collapsed shows only a move pill (on-the-block-card.tsx:111-145); (3) strategy switcher ABOVE the on-block card and RANK-ORDERED #1..#5 'from your research' - today it is below the card + unordered (strategy-strip.tsx, auction-room.tsx:338-345); (4) status bar shows round/pick 'R3 PICK 27' - today it has Leave/clock/league only, round-pick has no home (status-bar.tsx:45-102); (5) on-block headshot slot + positional rank ('RB6') in meta (on-the-block-card.tsx:111-127); (6) inline collapsible Players section (search + ALL/QB/RB/WR/TE/DEF filters + Rk/Player/Proj/Value/Fav) in the cockpit scroll per Joe 2026-08-18 - keep the Research tab too, they coexist. READ WIRING: (Opus) wire runMonteCarlo land-probability into the live Read after a LATENCY check (24 seeded runs/nomination synchronous may stall - precompute per nomination or cap runs); (Sonnet) plumb explain.ts HIGH/MED/LOW confidence onto the on-block card. MANUAL ENTRY: already shipped functional (ManualPickEntry variant='bar', offline-provisional submit/undo, client.tsx:605-621) - NO new build, just verify it covers the feed-down offline case; the old 'inert placeholder' note is retired. Per-session A1-A10 gate applies; screenshot limitation per WORKING_STATE.md, use DOM/network/test proof.",
