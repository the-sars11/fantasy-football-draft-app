<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    ...
    "R13 [Sonnet+Opus, L]: Dedicated bug hunt + test hardening - /bug-hunt full, real coverage on the new engines. If findings exceed one sitting, catalog here and split fixes into R13-fix cards. TEST-HARDENING PORTION DONE 2026-08-22 (Opus/Sonnet/Haiku by group): LIVE-DRAFT ADAPTIVE ENGINE suite, +52 tests (632->684 green) across Group A (flow-monitor/pivot-detector/auction-advisor units), Group B (recompute-on-pick + use-draft-state hook), Group C (auctioneer-feed/draft-sessions/players routes), Group D (Claude-driven Chrome on ?sim=1 with exact before/after DOM). 11 data-testids added to source. 0 engine defects; 2 sim-path defects cataloged for triage -> R13-fix: F1 (medium, tiers render 0 in sim so urgency-<POS> can't surface) + F2 (low, useUserTags 500 on non-UUID demo-league id). Full evidence: `.claude/TEST_FINDINGS.md`. /bug-hunt full portion still open.",
...
- [x] **PLAN-1 [Opus] · output+shared (Board REPOINT) — "Your Plan" board header · DONE 2026-08-23 (code green; push not yet flagged).** Joe-approved 2026-08-22: GAP is a bargain-detector, not a decision, so lead `/prep/board` with what the SIM buys and demote GAP to the existing `+X pocket`/`X hot` footnote chip. New restrained header panel ON the board page (not a new route) that derives entirely from the live dataset (nothing hand-entered): Anchor (most-frequent stud across top sim combos + top pairing record + walk-away price), Second big buy - pick one (the OTHER stud per top pairing, best-graded first, `best` chip), Pockets (pocket/sleeper tags the room underprices, deduped from the names already called out, INJ tag), Let-them-overpay strip (tax tags). New pure `src/lib/prep/draft-plan.ts` `buildDraftPlan(dataset)` uses the SAME rules the headless report uses (combos by `sim.grade.meanWins`, pockets by `valueGap` desc, tax by `valueGap` asc, `leagueIntel.positionalInflation`, solver-fit `walkUp`). Files: `src/lib/prep/draft-plan.ts` (+`.test.ts`, 6 tests), `src/components/prep/draft-plan-panel.tsx`, `src/app/(app)/prep/board/client.tsx`. Approved direction `.claude/mockups/board-plan-panel-direction.html`. **Proof:** type-check 0, test:run **711/711** (+6), lint 0 on new files; live render on my own dev server (port 3141) vs published Supabase snapshot — Gibbs anchor, WR second-buys w/ records+prices, INJ pockets, overpay strip, GAP as the `+$6 POCKET` footnote. Screenshot deferred (sandbox Browser pane does not composite; same constraint as every D/W/SP card) — folds into R15 visual sign-off. See CHANGELOG PLAN-1. **Remaining:** commit by explicit path; flag the Vercel-redeploy push before pushing.
...
- [x] **P2 · output (Design+QA) — per-screen IA rebuild inside SHIELD v4 · DONE 2026-08-22.** All four prep screens; render full 13-man rosters with the real dollar spread (Defect D); fix SHIELD violations (inline hex, volt-green `rgba(139,255,69,...)`, HTML-table player lists); expandable rows not card grids; mobile-first 44px.
  ...
  - [x] **P2 / Players intel card · DONE 2026-08-22.** `ffi-player-intel-card.tsx`: all 77 inline hex purged to `--ffi-*` tokens; the Joe-approved D4 expandable-row IA untouched (structure/logic byte-identical, only color values changed). Confirmed the earlier "opacity modifiers don't translate" worry was wrong -- Tailwind CSS 4 here DOES support `bg-[var(--ffi-blue)]/18` opacity-on-variable (15+ existing uses across settings/simulate/draft-setup). Exact-match swaps (blue/blue-bright/ink/ink-2/ink-3/volt) = zero visual change. Reconciled off-palette drift: `#ff716c`->`--ffi-danger` (near-identical coral), `#f5b301`->`--ffi-warning` (**visible: yellow->orange on volatile/injury tags**), `#8bacff`->`--ffi-blue-bright` (**visible-ish: periwinkle->cyan on sleeper/accents**), plus DEF/K entries added to `POS_CHIP`. Both visible shifts flagged to Joe for eyeball/veto. **Proof:** Grep 0 hex + 0 volt-green + 0 off-palette drift; type-check 0, lint 0, test:run 632/632. Live visual render NOT captured this session (Browser pane not displayed -> non-compositing -> screenshot/JS time out; all 3 API endpoints verified healthy 200). Presentational-only token swap; type-check guards the style objects.
...
> **Shipped:** `SwipeCarousel`/`EdgeSwipeBack` deleted with the file (`grep -rn "SwipeCarousel|EdgeSwipeBack" src/` = 0 both). `app-shell.tsx` mobile branch now renders `<PageTransition>` directly in `main` (no wrapper), same as the live room already did. `page-transition.tsx` `left`/`right` variants changed from x-axis slide to the same fade/scale as `fade` (kept as `Record<NavDirection,...>` keys since `nav-context.tsx` still emits `left`/`right` for drill-in/out and was out of scope to touch) -- kills the spring-slide feel without touching direction logic. **Scroll proof:** `document.scrollingElement.scrollHeight` (844) == `window.innerHeight` (844) on `/prep` because the app shell scrolls inside `<main class="overflow-y-auto">`, not the document -- that architecture predates this card and wasn't in scope. The real proof is `main.scrollHeight` (899) > `main.clientHeight` (732) on `/prep`, and on `/prep/board` (the screen with an actual player list, 483 rows) `main.scrollHeight` 43027 vs `clientHeight` 732; scrolled `main` to max (scrollTop 42295 = scrollHeight-clientHeight) and confirmed row #483 "Thomas Fidone II" `getBoundingClientRect()` = `{top:536.7, bottom:552.7}`, fully inside the 844px viewport. Horizontal swipe: synthetic pointerdown/move/up dragged 300px right-to-left on `/prep`'s `main`, pathname stayed `/prep` (no handler left after deletion, confirmed by the 0-grep). Gate: type-check 0, lint 30 errors/110 warnings (was 30/112, 0 new, -2 from the deleted file's own warnings), test:run 789/789 (65 files, unchanged), build "Compiled successfully in 4.0s" + all routes still statically prerendered. Commit `458aba6`.

### IA-0.2 -- Delete Strategies + Leaderboard + Sims-page (keep engines) `[Sonnet]` · class: shared -- **[x] DONE 2026-08-24 (code green, gate PASS, awaiting V-0.2)**
> - Class: WORKHORSE
...
### SP-1 -- Tier A token sweep: draft hub, draft/review, cheat sheet `[Sonnet]` · class: output -- **[x] DONE (2026-08-23)**
> **Shipped 2026-08-23:** literal line numbers had DRIFTED from the plan map (PLAN-1 board panel + prior sweeps moved them) and the board's volt-green was ALREADY gone -- so the real ground-truth set, captured by grep before editing, was: `review/client.tsx` green `rgba(139,255,69,...)` -> brick-red `rgba(166,60,65,...)` at :80,:81,:267 and off-token-blue glow `rgba(77,130,255,0.5)` -> steel-blue `rgba(95,168,224,0.5)` at :182; `board/client.tsx` off-token blue `rgba(77,130,255,...)` -> steel-blue `rgba(95,168,224,...)` at :361,:362,:470,:504,:560,:561 (no green present); `draft/page.tsx` `Zap` -> `Gavel` at import :24 + usage :333. Sanctioned position-chip hex untouched (sweep targeted only the 4 retired patterns, none of which are chip colors). Gate: done-when grep of all 3 files for `139,255,69`/`77,130,255`/`8bacff`/`Zap` = 0; em/en-dash check clean; `type-check` 0; scoped `eslint` exit 0; `test:run` 711 passed / 58 files (was 632 baseline, no regression); `build` Compiled successfully exit 0. Screenshot half deferred to SP-1V / Joe's eyeball via preview (non-compositing Browser-pane constraint in this sandbox).
> - Class: WORKHORSE
...
### SP-1V -- Validate Tier A sweep `[Sonnet · Opus if visual dispute]` · class: output -- **[x] DONE (2026-08-23)**
> **Validated 2026-08-23 (OTHER_FAMILY, fresh-context Sonnet sub-agent, did NOT write SP-1):** VERDICT PASS. Independent grep = 0 stale literals; diff review confirmed every green->brick-red and off-token-blue->steel-blue swap is alpha-exact (0.10/0.18/0.22/0.35/0.45/0.5 all held) with no semantic drift (steal/value stays red, structure/info stays steel-blue); Zap->Gavel in both import + JSX, Gavel confirmed a real lucide export, no unused import; position-chip palette `POS_PILL_COLORS` (board :46-52) untouched; gate re-run green (type-check 0, lint 0, test:run 711/711). One LOW finding D1: a THIRD off-token blue triple `rgba(121,166,255,...)` on the board league-name chip (:700-701) that SP-1's grep target list never caught (different RGB). D1 verified and FIXED inline in the same pass -> `rgba(95,168,224,...)` (alphas 0.10/0.20 held); extended grep (now incl. `121,166,255`/`79A6FF`) = 0; live DOM scan of /prep/board shows 0 retired literals. 0 unresolved HIGH. Screenshot/mobile pixel check deferred (Supabase unauthorized in sandbox -> board data 500s; no compositing).
...
### SP-7 -- Rebuild (auth) sign-in + sign-up to approved mockup `[Sonnet]` · class: output -- [x] DONE 2026-08-23
> - Class: WORKHORSE
...
> **DONE (proof):** all 3 files rebuilt against mockup screen 05. `(auth)/layout.tsx` -> navy `.stadium-atmos` + `.atmos-grain` field, shadcn `bg-background`/`blur-3xl` glow removed. `sign-in/page.tsx` + `sign-up/page.tsx` -> Oswald "FANTASY" wordmark over `.ffi-title-red` Kanit title, `.ffi-card-elevated .ffi-sheen` panel, `.ffi-input .ffi-form-input` red-volt fields, `FFIButton variant="hero"` brick CTA, steel-blue links; no shadcn `Card` remains. All Supabase wiring preserved (`useActionState` signIn/signUp, field names email/password/confirmPassword/fullName, hidden `redirect`, `useSearchParams` errors, `Suspense`, `state.success` "Check Your Email" branch). NEW additive `.ffi-sheen` class in globals.css (opt-in iridescent edge; nothing else affected). Gate: type-check 0; eslint 3 files clean; retired-literal grep (green + all off-token blues) = 0 across all 3; dash 0/0/0; test:run **749/749 (61 files)**; build clean, `/sign-in` + `/sign-up` both `○` prerendered static. Live render verified (DEV_MODE temporarily off + restart, then restored + confirmed /sign-in->/prep): both screens render with header/fields/wiring; computed styles confirm navy field (fixed z0 steel radial), elevated+sheen card (surface-3->surface-2, sheen ::before opacity 0.55), red Kanit gradient title (background-clip:text), brick rounded-full hero button, dual-class volt inputs.
> **Decisions (flagged):** (1) added additive shared-CSS `.ffi-sheen` rather than misuse `.ffi-hero`/`.ffi-card-interactive`; (2) `AuthHeader`/`Field` helpers duplicated inline in both pages to keep scope to the 3 plan-named files; (3) live verify needed a temporary DEV_MODE flip because middleware redirects authed users off auth routes (`src/lib/supabase/middleware.ts:19`). See CHANGELOG SP-7.

### SP-7V -- Validate (auth) rebuild `[Sonnet · Opus for visual parity]` · class: output -- [x] DONE 2026-08-23
> - Class: WORKHORSE (validation)
...
> **VERDICT: PASS -- 0 defects (0 CRIT/HIGH/MED/LOW).** OTHER_FAMILY fresh-context Sonnet, did not write SP-7. Independent grep of all 3 auth files + `.ffi-sheen` block: every retired literal (`139,255,69`/`8bff45`/`77,130,255`/`8bacff`/`121,166,255`/`79A6FF`/`4d82ff`) = 0; dashes 0/0/0. No shadcn `Card`/`blur-3xl` live (only in the layout.tsx:8 comment). Design Lens a-g all PASS vs mockup screen 05 (Oswald wordmark over `.ffi-title-red` Kanit gradient, `.ffi-card-elevated .ffi-sheen` panel both pages, dual-class `ffi-input ffi-form-input` on every field, live brick hero gradient `rgb(166,60,65)->rgb(110,34,37)`, steel-blue links, navy field mounted). QA Lens all PASS -- wiring preserved: `useActionState` signIn/signUp, hidden redirect, useSearchParams errors, Suspense, field names + minLength=8, `state.success` branch; imports resolve to real exports (actions.ts:32/37/72); no `any`. Gate re-run green: type-check 0, eslint 3 files clean, test:run **753/753 (61 files)**, build clean + `/sign-in`+`/sign-up` static. Live render confirmed both routes (DEV_MODE flipped + restored to true, `/sign-in`->`/prep` redirect verified back). No fixes needed. See CHANGELOG SP-7V.
