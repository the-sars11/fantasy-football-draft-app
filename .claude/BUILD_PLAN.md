<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    "UX-S1 + UX-S2 done (plan + 4-tab nav skeleton, verified desktop+mobile). NEXT: UX-S2.5 per-screen UX layout blueprint (hero/separation/flow for every screen), then UX-S3 Research tab. See .claude/UX_OVERHAUL_2026-08.md.",
...
- [ ] UX-S2.5: Per-screen UX layout blueprint — before building each tab's content, define for EVERY screen (Research, Draft live, Review, Setup + their sub-screens): the ONE hero element, top-to-bottom section order, the single primary action, what's demoted to secondary, empty/loading/error states, and the tap-flow to get in and out. Goal: good hero, clean separation, obvious flow, intuitive + easy to navigate. Output: a per-screen section appended to `.claude/UX_OVERHAUL_2026-08.md`. This is the spec UX-S3..S6 build to — no screen gets built without its blueprint signed off. | `docs`
