<!-- DASHBOARD_STATUS
{
  ...
  "nextItems": [
    ...
    "[DONE 2026-08-12] S5 [Sonnet]: bug hunt + test hardening on S1-S4 -- /bug-hunt full (0 CRITICAL/HIGH), 43 new tests (162->205). Extracted dbLeagueToAppLeague to league-mapper.ts (BUG-004 testability). All gates green.",
...
### S5 — Bug hunt + test hardening (S1-S4) `[Sonnet · Opus for logic bugs]` · class: bugfix/pipeline -- **[DONE 2026-08-12]**
...
> **Closed 2026-08-12:** type-check clean · 205/205 tests (was 162, +43 new) · lint 161 problems (1 fewer than 162 baseline, 0 new) · build green. Static analysis: 0 CRITICAL/HIGH bugs; 1 LOW (BUG-004, untestable `dbLeagueToAppLeague` -- extracted to `league-mapper.ts`, now covered by 11 tests). New tests: 2-file extractions (league-mapper.ts + league-mapper.test.ts, auction-advisor.test.ts with 16 VAL-3 calibrated-anchor tests) + extensions to value-range.test.ts (BUG-001 regression), tags.test.ts (VOLATILE/SLEEPER boundary + multi-tag), headshot.test.ts (normalizeName edge cases + null-fallback pattern).
