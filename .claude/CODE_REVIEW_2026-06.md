...
| 12 | P1 | UX / mechanics | live/client.tsx, manual-pick-entry.tsx | No edit/correct of a mis-entered pick; only LIFO undo. | PARTIAL (2026-08-09, 2e48c84) - Stage A (logic) done: pure removePickByNumber/editPickByNumber in state.ts + editPick/removePick hook actions that rebuild from scratch (fresh managers -> re-apply keepers -> replay) so budgets/roster/snake-turn stay consistent; 10 new unit tests. Stage B (tap-a-pick edit/remove UI) BACKLOG - substantial UI, needs design sign-off before build. |
