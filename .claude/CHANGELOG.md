...
- **`src/components/draft/live-room/budget-strip.tsx` (rewritten):** new `leagueBudget?: number` prop (default 200); old red-to-green gradient bar replaced with a steel-blue progress fill (`role="progressbar"`, `ROOM.muted25` track / `ROOM.blue` fill) computed from `remaining / leagueBudget`, clamped 0-100.
