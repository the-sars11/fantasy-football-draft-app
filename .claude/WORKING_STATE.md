...
## Current Session
- **Date:** 2026-06-04
- **Focus:** UX-7.1 — Dev-only Sim engine
- **Status:** COMPLETE. `src/hooks/use-draft-simulator.ts` (new); `live/client.tsx` wired. Navigate to `/draft/live?session=<id>&sim=1` in dev to activate. type-check clean, 29/29 tests, 0 net-new lint errors, build passes. Sim HUD visual deferred (requires active session to render the guarded return path).
- **NEXT:** UX-7.2 — Drive every signature moment + force AI advisor off in sim. Or FF-313 if Joe picks Option D or B.
- **GATED:** live AI auto-fire needs `ANTHROPIC_API_KEY` + typed cost approval. FF-313 awaits Joe's approach pick (D vs B). FFT-008 needs Joe on phone.
