...
## Current Session
- **Date:** 2026-08-09
- **Focus:** UXV2-7 — reduced-motion (dial-down) + performance / arm's-length pass on the live auction room.
- **Status:** UXV2-7 LANDED. Added the `ffi-live-room` class to the room root (`auction-room.tsx`, no layout change) and a scoped `@media (prefers-reduced-motion: reduce)` block in `globals.css` that DIALS DOWN, not strict-off: cross-fades stay but halve to 75ms, and transform tap-feedback (`active:scale-90/95` on the Research star toggles + RECORD button) is neutralized (`transform: none`). The LIVE-dot pulse was already gated by its `motion-safe:` variant. Perf: the room ships lean by construction - a live-DOM audit across 735 room elements found 0 CSS filter layers, 0 backdrop-filter layers, 0 animated box-shadows, and 0 elements holding `will-change`, so there is nothing to promote/release and the old build's heavy-filter-stack non-compositing failure is absent. Mobile 375: no horizontal overflow, room fits 343px, primary decision text 15-22px (8.5-9.5px items are the locked v4 uppercase micro-labels / position badges, left as-is). VERIFIED: `tsc --noEmit` 0 errors, ESLint clean on the changed `.tsx`, `npm run test:run` 40/40, `npm run build` clean (`/draft/live` in route list); live computed-style proof on the running dev server via a media-rule toggle (transition-duration 0.15s -> 0.075s on real cross-fade + transform-transition elements; the transform-neutralize rule is present and matches all 80 ResearchView tap buttons). No paid endpoints fired.
- **NEXT:** UXV2-8 (final VERIFY: lint + tests + build + docs).
