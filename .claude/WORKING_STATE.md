...
**Next open item:** **DR-3 [Sonnet] -- Fix the inverted cost-guard** (confirm-gate Generate Strategies + board Refresh which actually spend Claude; remove misleading credit warning from the deterministic Run Research). Class: pipeline. Dependency: DR-2 (done).
...
**Recently landed (pointer; detail in CHANGELOG):** DR-2 dead-path removal 2026-08-10 -- deleted Google Sheets lib/route/hook, snake/keeper UI surfacing, 3 orphaned files; edited 13 source files; all 96 tests pass, 0 type errors, 0 new lint errors, build clean.
