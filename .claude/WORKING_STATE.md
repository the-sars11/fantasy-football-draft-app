...
- **Status:** LIVE ROOM LANDED (part 1). New `src/components/draft/live-room/` room + `src/lib/draft/what-to-do.ts` decision brain (11 tests) wired into `live/client.tsx` via an early `if (isAuction) return <AuctionDraftRoom .../>` branch. Tyler's snake path is byte-for-byte unchanged; secondary panels moved into a mount-on-open "More tools" section so no paid `/api/draft/recommend` fires until opened. Guards added: tier NaN → UNRANKED/NR, missing bye omitted, $0 caps floored to the $1 auction minimum. VERIFIED: `npm run build` clean (`/draft/live` in route list), 11/11 what-to-do tests, `tsc --noEmit` 0 errors, live DOM proof of every section + the On-the-Block/What-To-Do flow in sim. NOT YET committed at time of writing (committing this milestone by explicit path).
- **NEXT:** UXV2-6 part 2 — Research-tab draft-mode screen (mini on-block strip + inline record, target view, tappable tier chips; Recent Sales removed). Then UXV2-7 (perf/arm's-length + reduced-motion) and UXV2-8 (final VERIFY + docs).
