...
- **Status:** FF-280 COMPLETE — BroadcastChannel subscriber added to `useAuctioneerfeed`. Instant pick sync when Auctioneer and FFI run in the same browser (localstorage path only). Poll still runs as catch-up. Auctioneer publisher was already in place from AA-INT1. Next: FF-281 dedup merge.
