...
## Current Session
- **Date:** 2026-08-09
- **Focus:** UXV2-6 — Live Auction Draft Room rebuild (v4 mockup approved).
- **Status:** LIVE ROOM LANDED (parts 1 + 2). Part 1 = the auction room + `what-to-do.ts` decision brain. Part 2 (this session) = the Research tab as an INTERNAL room view (`src/components/draft/live-room/research-view.tsx`): sticky on-the-block mini strip with inline record (reuses `addManualPick`), position pills + ★ Target View filter bar, available player list (star toggle, position badge, real-data-only signal chip, tier chip, range or `AVOID`), and a tappable Tier Context that filters the list. Recent Sales removed per v4 sign-off. Bottom nav switches Research/Draft in place (`onSelectView`); Review/Setup still navigate. Same dev-cache guards (tier→NR, ranges floored to $1, bye omitted). Fixed a nested-button hydration error found during verify (star + tap-target are now sibling buttons). VERIFIED: `tsc --noEmit` 0 errors, 5 changed files lint-clean, `npm run test:run` 40/40, `npm run build` clean (`/draft/live` in route list), live DOM proof of the view switch, tap-to-block, inline RECORD firing `addManualPick`, position + Target View filters, and tier-context filtering in sim. Committed this session by explicit path.
- **NEXT:** UXV2-7 (perf/arm's-length + reduced-motion) and UXV2-8 (final VERIFY + docs).
