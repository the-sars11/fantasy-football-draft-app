...
**R6 [Opus] — Wire the solver into STRATEGY target prices.** Each strategy assigns a target $ per target player via the solver so the full roster fits $200; swapping archetype re-allocates the money. Reads first: `src/lib/research/strategy/*`, `roster-solver.ts`, `components/prep/strategy-proposals.tsx`. See `BUILD_PLAN.md` R6.
