...
## Current Session
- **Date:** 2026-06-04
- **Focus:** FF-313 — App-shell double-mount fix (Option D)
- **Status:** COMPLETE. `useIsMobile` hook (new `src/hooks/use-is-mobile.ts`) + `app-shell.tsx` single-render refactor. DOM count confirmed 1 at both 1280px + 375px. type-check clean, 29/29 tests, 0 net-new lint errors, build passes. Committed + pushed.
- **NEXT:** All P0 items done. Remaining items gated: FFT-006/007/FF-072 need cost approval; FFT-008 needs Joe on phone; FF-260 blocked until real Aug 2026 draft sheet; FF-080/081 are draft-day actions. P3+ gated on unmet conditions.
