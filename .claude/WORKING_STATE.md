...
## Current Session
- **Date:** 2026-06-03
- **Focus:** UX-6.1 — Empty states + skeletons across remaining screens
- **Status:** UX-6.1 shipped. `page-skeleton.tsx` upgraded from v1 shadcn Skeleton to v2.0 `.ffi-skeleton` shimmer + dark surface tokens. Inline `Loader2` spinners in runs + strategies clients replaced with shimmer row skeletons. Empty states upgraded from generic cards (`bg-card border-border`) to glass surface v2.0 (`bg-[#0a1b25] border-white/[0.04]`). Runs + strategies page headers match configure's `ffi-display-md` v2.0 pattern. type-check clean, 29/29 tests, 0 net-new lint errors, build passes.
- **GATED:** live AI auto-fire end-to-end needs `ANTHROPIC_API_KEY` in `.env.local` + Joe's typed cost approval (~cheap Haiku per pick). FF-313 app-shell double-mount awaits Joe's approach pick (D vs B).
