...
## Current Session
- **Date:** 2026-06-03
- **Focus:** UX-6.3 — Background-layer GPU promotion
- **Status:** UX-6.3 shipped. `globals.css`: `.atmos-grain` gets `transform: translateZ(0)` (own GPU compositor layer). `.stadium-atmos.atmos-clock` + `body.ffi-on-the-clock .stadium-atmos` get `will-change: filter` (filter-animation GPU hint). Reduced-motion block resets `will-change: auto` on those two selectors to release GPU memory when animations are suppressed. type-check clean, 29/29 tests, 0 net-new lint errors, build clean.
- **GATED:** live AI auto-fire end-to-end needs `ANTHROPIC_API_KEY` in `.env.local` + Joe's typed cost approval (~cheap Haiku per pick). FF-313 app-shell double-mount awaits Joe's approach pick (D vs B). FFT-008 arm's-length physical test still needs Joe on phone.
